...
function WechatStrategy(options, verify) {
  ...
  this._verify = verify;
  if (global.loopbackApp && global.loopbackApp.cacheRedisCache) {
    this._oauth = new OAuth(options.clientID, options.clientSecret, function (openid, cb) {
      loopbackApp.cacheRedisCache.getJSONAsync('wechat:access_token:' + options.clientId)
      .then(function (val) {
        debug('get token from cache', val);
        cb(null, val);
      }, function (err) {
        console.dir(err);
        cb();
      ...
      loopbackApp.cacheRedisCache.putJSON('wechat:access_token:' + options.clientId, token, 1000 * 60 * 60);
      cb(null, token);
    });
  } else {
  ...
  if (req.query && req.query.code) {
    ...
    self._oauth.getAccessToken(code, function (err, response) {
      ...
      if (~params.scope.indexOf('snsapi_base')) {
        ...
        res.cookie('wx:openid', profile.openid);
        try {
    ...
    var location = self._oauth[methodName](callbackURL, state, self._scope);
