...
function WechatStrategy(options, verify) {
  ...
  this._verify = verify;
  if (global.loopbackApp) {
    this._oauth = new OAuth(options.clientID, options.clientSecret, function (openid, cb) {
      this.cache.getJSONAsync('wechat:access_token:' + options.clientId)
      .then(function (val) {
      ...
      this.cache.putJSON('wechat:access_token:' + options.clientId, token, 1000 * 60 * 60);
      cb(null, token);
    });
  } else {
