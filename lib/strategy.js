...
function WechatStrategy(options, verify) {
  var cache = global.loopback && global.loopback['app_' + global.ROOT_NAME_SPACE] && global.loopback['app_' + global.ROOT_NAME_SPACE].cacheRedisCache
  options = options || {};
  ...
  if (cache) {
    this._oauth = new OAuth(options.clientID, options.clientSecret, function (openid, cb) {
      cache.getJSONAsync('wechat:access_token:' + options.clientId)
      .then(function (val) {
      ...
      cache.putJSON('wechat:access_token:' + options.clientId, token, 1000 * 60 * 60);
      cb(null, token);
    });
  } else {
